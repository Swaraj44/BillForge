...
import Backend.GeneralDataProcessing as GeneralDataProcessing
import time
import hashlib
...
import threading
import Backend.EmailProcessing as EmailProcessing

import datasave as save_data


import GUI_Login_ForGeneralDataProceaaingDashboard as GL
import GUI_Login_For_BacklogDataProcessing as BLGLI
...
# Sample objects to be saved
  ...
  obj= Passward_Class(name=name, value=value)
 ...
 def SignUp():
    ...
    if entered_username is not None:
      if entered_password is not None:
          save_data.savedata(entered_username,entered_password)
...
# Create the SignUp window
 SignUp_window = tk.Tk()
 SignUp_window.title("BillForge-Signup Page")
 SignUp_window.iconbitmap(resource_path('data\\images\\general_logo.ico'))
