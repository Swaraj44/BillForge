...
import datasave as Store_data
import showdata as Fetch_data
import pandas as pd
import tkinter as tk

import Backend.Digit_To_BanglaTaka_ConverterApi as tkconvert
...
import Backend.Year_Extractor_Module as ys



List_of_Teachers_Name = []
...
Script_Examiner_combined_balance_map = {}

examiners_class_tests_map = {}

examiners_class_tests_students_in_plus_map = {}
...
def is_underlined(run):
    """
    Cheflag if a run contains underlined text.
    """
...
#Fuction that returns the numner of teachers in a map
def size(map):
    flag = 0
    for teacher in List_of_Teachers_Name:
        x = map.get(teacher, 0)
        if x != 0:
          flag += 1
    ...
    for i, table in enumerate(doc.tables, start=1):
        ...
        if i == 1:
            ...
            for row in table.rows:
                ...
                List_of_Teachers_Name.append(data)
        ...
        if i == 2:
            ...
            for row in table.rows:
                ...
                if data == '':
                  ...
                  if data not in List_of_Teachers_Name:
        ...
        if i == 3:
            ...
            for row in table.rows:
                ...
                if data_r == "Same as paper setter" or data_r == "Same as Paper Setter" or data_r == "same as paper setter":
                    ...
                    ################################################ 
                    num_of_ans_sheet = int(data2)
                    prev=Script_Examiner_combined_balance_map.get(data,0)
                    
                    global minimum_script_charge 
                    global per_student_script_charge
                    
                    if num_of_ans_sheet*per_student_script_charge > minimum_script_charge:
                       Script_Examiner_combined_balance_map[data]=prev+num_of_ans_sheet*per_student_script_charge;
                       
                    else :
                        Script_Examiner_combined_balance_map[data]=prev+minimum_script_charge;
                    ...
                    ##############################################################
                    num_of_ans_sheet = int(data2)
                    prev=Script_Examiner_combined_balance_map.get(data_r,0)
                    
        
                    
                    if num_of_ans_sheet*per_student_script_charge > minimum_script_charge:
                       Script_Examiner_combined_balance_map[data_r]=prev+num_of_ans_sheet*per_student_script_charge;
                       
                    else :
                        Script_Examiner_combined_balance_map[data_r]=prev+minimum_script_charge;
                    
                    #############################################################
                    
                    
                    course_collection_1 = Script_Examiner_course_collection_str.get(data_r, "")
                    if course_collection_1=="":
                            Script_Examiner_course_collection_str[data_r] =str(data_cr)
                    else:
                            Script_Examiner_course_collection_str[data_r] =course_collection_1+","+str(data_cr)
                    
                    
                    if data_r not in List_of_Teachers_Name:
        ...
        if i == 4:
            ...
            for row in table.rows:
                ...
                if et==0:
                    examiners_class_tests_students_in_plus_map[data]=str(data2)
                elif et>0:
                    examiners_class_tests_students_in_plus_map[data]=examiners_class_tests_students_in_plus_map[data]+"+"+str(data2)
                ...
                if int(data2)>0:
                 print("Test---------------->",examiners_class_tests_students_in_plus_map[data])
                #theory_course_map[data] = data3
                
                print("-------------------------------Pls------>",data," ----- ",data2);
...
def excel_add(Teachers_Instance):
    workbook = openpyxl.load_workbook(resource_path('data\\excel-files\\templatebill.xlsx'))
    name_string = "নাম: "
    podobi_string = "পদবী: "
    ...
    new_value = Teachers_Instance.name
    #/////////////////////////////////////////////////////////////////////////Turjo///////////////////////////////////////////////////
    st1 = "Null"
    if Teachers_Instance.name in bangla_teachers_name:
      st1 = bangla_teachers_name[Teachers_Instance.name]

    
    if(st1!="Null"):
         new_value = bangla_teachers_name[Teachers_Instance.name]
    #print("New Value is : ", new_value)
         merged_cell[0][0].value = name_string + new_value
    else:
        temp_str="Bangla Teacher's Name Mismatching! "+Teachers_Instance.name
    ...
    new_value = bangla_teachers_designation[Teachers_Instance.designation]
    #print("New Value is : ", new_value)
    ...
    new_value = Teachers_Instance.dept
    ...
    if Teachers_Instance.name in teacher_salary_id_map:
            
        cell = sheet.cell(row=3, column=5)
        z=teacher_salary_id_map[Teachers_Instance.name]
        cell.value = z
    ...
    #cell.value = Teachers_Instance.question_paper_setter
    
    if Teachers_Instance.name in teacher_exampaper_course_collection_str: 
       cell = sheet.cell(row=12, column=7)
       cell.value = teacher_exampaper_course_collection_str[Teachers_Instance.name]
    
    
    #Script_Examiner_combined_balance_map
    if Teachers_Instance.name in Script_Examiner_combined_balance_map:
        cell = sheet.cell(row=12, column=10)
        cell.value = Script_Examiner_combined_balance_map[Teachers_Instance.name]
        
   
    
    #print("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT",Teachers_Instance.question_paper_setter)
    ...
    #Clss Tests
    if Teachers_Instance.name in examiners_class_tests_students_in_plus_map:
         cell = sheet.cell(row=14, column=7)
         cell.value = examiners_class_tests_students_in_plus_map[Teachers_Instance.name]
    
    #Errornous Code:
    # if Teachers_Instance.name in teacher_classtest_course_collection_str:
    #     str1=teacher_classtest_course_collection_str[Teachers_Instance.name] + "="+str(Teachers_Instance.examiners_class_tests)
    #     cell.value = str1
    
    
    # Seminar 
    cell = sheet.cell(row=16, column=7)
    temp1=Teachers_Instance.seminar
    cell.value = Teachers_Instance.seminar
    ...
    #       flag =size(seminar_map)
    #       cell.value=flag
          
    if temp1 >0:
          cell = sheet.cell(row=16, column=8)
          v1 =Teachers_in_seminar_group[Teachers_Instance.name]
          cell.value=v1
    


     
    if Teachers_Instance.name in Script_Examiner_course_collection_str:
            
            cell = sheet.cell(row=12, column=5)
            z=Script_Examiner_course_collection_str[Teachers_Instance.name]
            cell.value = z
    ...
    st1 = "Null"
    if Teachers_Instance.name in teacher_classtest_course_collection_str:
      st1 = teacher_classtest_course_collection_str[Teachers_Instance.name]
    ...
    if(st1!="Null"):
        ...
        #s1=Teachers_Instance.theory_course
        s1=teacher_classtest_course_collection_str[Teachers_Instance.name]
        result = s1.split(',')
        ...
        cell.value = teacher_classtest_course_collection_str[Teachers_Instance.name]
        ...
        #z=Teachers_Instance.theory_course
        z=teacher_classtest_course_collection_str[Teachers_Instance.name]
        cell.value = z
        
        
    #Class Test Number
    
    if Teachers_Instance.name in class_test_map:
    
       cell = sheet.cell(row=14, column=8)
       cell.value = class_test_map[Teachers_Instance.name]
       
       
    if Teachers_Instance.name in class_test_map:
       cell = sheet.cell(row=14, column=10)
       cell.value = Teachers_Instance.examiners_class_tests*payment_per_student_class_tests;
    
    
    
    #sessional_per_student_charge
    if Teachers_Instance.name in examiners_sessional_classes_bill:
        cell = sheet.cell(row=17, column=10)
        cell.value = examiners_sessional_classes_bill[Teachers_Instance.name]
        
    
    y = Teachers_Instance.sessional_course
    ...
    if Teachers_Instance.name in teacher_sessional_course_collection_str :
        
        cell = sheet.cell(row=17, column=7)
        z=teacher_sessional_course_collection_str[Teachers_Instance.name]
        cell.value=z
        
        
    if Teachers_Instance.name in teacher_sessional_courses_name :
        
        cell = sheet.cell(row=17, column=5)
        z=teacher_sessional_courses_name[Teachers_Instance.name]
        cell.value=z
    ...
    cell.value = Teachers_Instance.central_viva_voce
    #tp1=size(central_viva_voce_map)
    #cell.value = tp1
    temp1 = Teachers_Instance.central_viva_voce
    
    
    cell = sheet.cell(row=18, column=8)
    flag=  size(central_viva_voce_map)
    if ( temp1 !=0):
        cell.value = flag
    else:
        cell.value = 0
    ...
    cell.value = Teachers_Instance.thesis_progress_defense
    temp1= Teachers_Instance.thesis_progress_defense
    
    #Here temp1 variable is used for cheflaging whether the left side value is zero 
    #or not if zero then this cell value will be zero
    cell = sheet.cell(row=20, column=8)
    flag = size(thesis_progress_defense_map)
    if (temp1 != 0):
        cell.value = flag
    else:
        cell.value = 0
    ...
    cell.value = Teachers_Instance.tabulation
    ...
    cell.value = Teachers_Instance.script_scrutinizer
    
    
    # List of Duty
    cell = sheet.cell(row=26, column=7)
    cell.value = Teachers_Instance.list_of_duty
    
    
    #Typing & Drawing
    
    cell = sheet.cell(row=27, column=7)
    cell.value = Teachers_Instance.typing_and_drawing
    
    
    #Final Grade Sheet Verification
    
    cell = sheet.cell(row=28, column=7)
    cell.value= Teachers_Instance.final_grade_sheet_verification
    ...
    cell.value = Teachers_Instance.student_advising
    
    
    #######################################################################################-----------> Sw
    st1 = 0
    if Teachers_Instance.name in Course_Coordinator:
      st1 = Course_Coordinator[Teachers_Instance.name]
    ...
    if (Question_Paper_Moderation_Board[Teachers_Instance.name]== 1):
        cell =sheet.cell(row=10, column=7)
        cell.value=1
        cell =sheet.cell(row=11,column=7)
        #previously this value was 5
        cell.value=1
        flag =size(Question_Paper_Moderation_Board)
        
        print("Moderation Board size is :......")
        print(Teachers_Instance.name)
        print(Question_Paper_Moderation_Board[Teachers_Instance.name])
        print(flag)
        flag = int(flag)
        cell =sheet.cell(row=11,column=10)
        moderate_taka = float((10*3600)/(1.00*flag))
    ...
    elif (Question_Paper_Moderation_Board[Teachers_Instance.name]== 2):
        cell =sheet.cell(row=11,column=7)
        cell.value=1
        flag =size(Question_Paper_Moderation_Board)
        flag = int(flag)
        ...
        dummy=10 
        #this is maybe of some  use in future
        
  
#################################################################
    
    # Saving the name to the respective teachers
    file_name = f"{Teachers_Instance.name}.xlsx"
    Individual_Teachers_Generated_xl_file_path=output_path+"/"+file_name
    workbook.save(Individual_Teachers_Generated_xl_file_path)
    
    
    ###############Timer Is USed here################
    time.sleep(0.001)
    excel_app = xlwings.App(visible=False)
    excel_book = excel_app.books.open(Individual_Teachers_Generated_xl_file_path)
    excel_book.save()
    excel_book.close()
    excel_app.quit()
    
    loaded_workbook = openpyxl.load_workbook(Individual_Teachers_Generated_xl_file_path, data_only=True)
    ...
    #Store_data.savedata(Teachers_Instance.name,formatted_value)
    Store_data.savedata2(Teachers_Instance.name,formatted_value,"summarydb")
    ...
    loaded_workbook.save(Individual_Teachers_Generated_xl_file_path)
    
    
    workbook = openpyxl.load_workbook(Individual_Teachers_Generated_xl_file_path)
    sheet = workbook.active
...
def KB():
    ...
    #print(Teachers_Instance.designation)
...
def summarygenerator():
     ...
     for teacher in teachers_array:
        ...
        ss=Fetch_data.showdata2(teacher.name,"summarydb") # Fetching  Data From Database Showing the teacher's total amount
        print("value: "+ss)
...
def main():
    ...
    num_teachers = len(List_of_Teachers_Name)
    
    flag = size(sessional_course_map)
...
# Accessing and printing information from the array of teachers
    for teacher in teachers_array:
        ...
        print(teacher.name)
        #print(List_of_Teachers_Name)
        
        
        
        nameee=teacher.name
        #print(teacher.sessional_course)
        if nameee in List_of_Teachers_Name:
           excel_add(teacher)
